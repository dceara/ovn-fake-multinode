...
function start-ovn-ic() {
    if [ "$OVN_START_IC_DBS" = "yes" ]; then
	    ...
	    [ "$OVN_DB_CLUSTER" = "yes" ] && CENTRAL_IC_ID="${CENTRAL_NAMES[0]}-1" || CENTRAL_IC_ID="${CENTRAL_NAMES[0]}"
        fi
...
ovn-nbctl --no-leader-only ls-add sw0\$az

# ovn dhcpd on sw0\$az
ovn-nbctl --no-leader-only set logical_switch sw0\$az \
  other_config:subnet="1\$az.0.0.0/24" \
  other_config:exclude_ips="1\$az.0.0.1..1\$az.0.0.2"
ovn-nbctl --no-leader-only dhcp-options-create 1\$az.0.0.0/24
CIDR_UUID=\$(ovn-nbctl --no-leader-only --bare --columns=_uuid find dhcp_options cidr="1\$az.0.0.0/24")
ovn-nbctl --no-leader-only dhcp-options-set-options \$CIDR_UUID \
  lease_time=3600 \
  router=1\$az.0.0.1 \
  server_id=1\$az.0.0.1 \
  server_mac=c0:ff:ee:00:00:01

ovn-nbctl --no-leader-only lsp-add sw0\$az sw0\$az-port1
ovn-nbctl --no-leader-only lsp-set-addresses sw0\$az-port1 "50:5\$az:00:00:00:03 1\$az.0.0.3 100\$az::3"
ovn-nbctl --no-leader-only lsp-add sw0\$az sw0\$az-port2
ovn-nbctl --no-leader-only lsp-set-addresses sw0\$az-port2 "50:5\$az:00:00:00:04 1\$az.0.0.4 100\$az::4"

# Create ports in sw0\$az that will use dhcp from ovn
ovn-nbctl --no-leader-only lsp-add sw0\$az sw0\$az-port3
ovn-nbctl --no-leader-only lsp-set-addresses sw0\$az-port3 "50:5\$az:00:00:00:05 dynamic"
ovn-nbctl --no-leader-only lsp-set-dhcpv4-options sw0\$az-port3 \$CIDR_UUID
ovn-nbctl --no-leader-only lsp-add sw0\$az sw0\$az-port4
ovn-nbctl --no-leader-only lsp-set-addresses sw0\$az-port4 "50:5\$az:00:00:00:06 dynamic"
ovn-nbctl --no-leader-only lsp-set-dhcpv4-options sw0\$az-port4 \$CIDR_UUID

# Create the second logical switch with one port
ovn-nbctl --no-leader-only ls-add sw1\$az
ovn-nbctl --no-leader-only lsp-add sw1\$az sw1\$az-port1
ovn-nbctl --no-leader-only lsp-set-addresses sw1\$az-port1 "40:5\$az:00:00:00:03 2\$az.0.0.3 200\$az::3"

# Create a logical router and attach both logical switches
ovn-nbctl --no-leader-only lr-add lr\$az
ovn-nbctl --no-leader-only lrp-add lr\$az lr\$az-sw0\$az 00:0\$az:00:00:ff:01 1\$az.0.0.1/24 100\$az::a/64
ovn-nbctl --no-leader-only lsp-add sw0\$az sw0\$az-lr\$az
ovn-nbctl --no-leader-only lsp-set-type sw0\$az-lr\$az router
ovn-nbctl --no-leader-only lsp-set-addresses sw0\$az-lr\$az router
ovn-nbctl --no-leader-only lsp-set-options sw0\$az-lr\$az router-port=lr\$az-sw0\$az

ovn-nbctl --no-leader-only lrp-add lr\$az lr\$az-sw1\$az 00:0\$az:00:00:ff:02 2\$az.0.0.1/24 200\$az::a/64
ovn-nbctl --no-leader-only lsp-add sw1\$az sw1\$az-lr\$az
ovn-nbctl --no-leader-only lsp-set-type sw1\$az-lr\$az router
ovn-nbctl --no-leader-only lsp-set-addresses sw1\$az-lr\$az router
ovn-nbctl --no-leader-only lsp-set-options sw1\$az-lr\$az router-port=lr\$az-sw1\$az

ovn-nbctl --no-leader-only ls-add public\$az
ovn-nbctl --no-leader-only lrp-add lr\$az lr\$az-public\$az 00:0\$az:20:20:12:13 172.16.\$az.100/24 300\$az::a/64
ovn-nbctl --no-leader-only lsp-add public\$az public\$az-lr\$az
ovn-nbctl --no-leader-only lsp-set-type public\$az-lr\$az router
ovn-nbctl --no-leader-only lsp-set-addresses public\$az-lr\$az router
ovn-nbctl --no-leader-only lsp-set-options public\$az-lr\$az router-port=lr\$az-public\$az

# localnet port
ovn-nbctl --no-leader-only lsp-add public\$az ln-public\$az
ovn-nbctl --no-leader-only lsp-set-type ln-public\$az localnet
ovn-nbctl --no-leader-only lsp-set-addresses ln-public\$az unknown
ovn-nbctl --no-leader-only lsp-set-options ln-public\$az network_name=public

# schedule the gw router port to a chassis.
ovn-nbctl --no-leader-only lrp-set-gateway-chassis lr\$az-public\$az ovn-gw-\$az 20

# Create NAT entries for the ports

# sw0\$az-port1
ovn-nbctl --no-leader-only lr-nat-add lr\$az dnat_and_snat 172.16.\$az.110 1\$az.0.0.3 sw0\$az-port1 30:5\$az:00:00:00:03
ovn-nbctl --no-leader-only lr-nat-add lr\$az dnat_and_snat 300\$az::c 100\$az::3 sw0\$az-port1 40:5\$az:00:00:00:03
# sw1\$az-port1
ovn-nbctl --no-leader-only lr-nat-add lr\$az dnat_and_snat 172.16.\$az.120 2\$az.0.0.3 sw1\$az-port1 30:5\$az:00:00:00:04
ovn-nbctl --no-leader-only lr-nat-add lr\$az dnat_and_snat 300\$az::d 200\$az::3 sw1\$az-port1 40:5\$az:00:00:00:04

# Add a snat entry
ovn-nbctl --no-leader-only lr-nat-add lr\$az snat 172.16.\$az.100 1\$az.0.0.0/24
ovn-nbctl --no-leader-only lr-nat-add lr\$az snat 172.16.\$az.100 2\$az.0.0.0/24

if [ "\$ic" == "yes" ]; then
    ovn-nbctl --no-leader-only lrp-add lr\$az lr\$az-ts1 aa:aa:aa:aa:aa:0\$az 5.0.0.\$az/24
    ovn-nbctl --no-leader-only lsp-add ts1 ts1-lr\$az -- lsp-set-addresses ts1-lr\$az -- lsp-set-type ts1-lr\$az router -- lsp-set-options ts1-lr\$az  router-port=lr\$az-ts1
    ovn-nbctl --no-leader-only lrp-set-gateway-chassis lr\$az-ts1 ovn-gw-\$az 1
fi
...
EOF
    ...
    while sleep 2; do
        ${RUNC_CMD} exec ${CENTRAL_IC_ID} ovn-nbctl --no-leader-only ls-list | grep -q ts1 && break
    done
